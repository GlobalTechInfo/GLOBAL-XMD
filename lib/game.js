...
if (teksnya.no === 0) {
	...
	} else m.reply(`Please wait *⏱️${timers}* again to be able to play again`)
}
...
const buy = async (m, args, db) => {
	if (args[0] === 'limit') {
		...
		if (db.users[m.sender].money >= count * 500) {
			...
			m.reply(`Successfully purchased ${args[1] * 1} limits for the price of ${args[1] * 500}`);
} else m.reply(`You don't have enough money to buy limits!\nYour remaining money: ${db.users[m.sender].money}\nPrice for ${args[1]} limits: ${args[1] * 500}`);
...
const transfer = async (m, args, db) => {
	if (args[0] == 'limit') {
		...
		if (!who) return m.reply('Who wants to be transferred??')
		if (db.users[who]) {
