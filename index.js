...
async function startGlobalBot() {
	...
	GlobalTechInc.ev.on('call', async (call) => {
		...
		if (anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await GlobalTechInc.sendMessage(id.from, { text: `Currently, We Cannot Receive Calls ${id.isVideo ? 'Video' : 'Suara'}.\nIf @${id.from.split('@')[0]} Need Help, Please Contact Owner :)`, mentions: [id.from]});
	...
	GlobalTechInc.ev.on('group-participants.update', async (update) => {
		await GroupParticipantsUpdate(GlobalTechInc, update);
	});
